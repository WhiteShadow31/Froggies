...
{
    [CustomPropertyDrawer(typeof(PredefinedValuesOnlyAttribute))]
    public class PredefinedValuesOnlyAttributeDrawer : PropertyDrawer
    {
        public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
        {
            // Get current attribute
            PredefinedValuesOnlyAttribute currentAttribute = attribute as PredefinedValuesOnlyAttribute;

            // Check if property type is supported, else log warning and return
            if (!IsTypeSupported(property))
            {
                Debug.LogWarning($"{nameof(PredefinedValuesOnlyAttribute)} of {property.name} in {property.serializedObject.targetObject.name}.{(property.serializedObject.targetObject as MonoBehaviour).GetType()} : {property.propertyType} type not supported (only work with float, int and string)");
            }

            // Check if param values are correct, else log warning and return
            if (!ValuesCorrect(currentAttribute.Values, property))
            {
                Debug.LogWarning($"{nameof(PredefinedValuesOnlyAttribute)} of {property.name} in {property.serializedObject.targetObject.name}.{(property.serializedObject.targetObject as MonoBehaviour).GetType()} : One or multiple param values are wrong (not the same type as variable)");
                return;
            }
            ...
            switch (property.propertyType)
            {
                ...
                case SerializedPropertyType.Float:
                    ...
                    int newFloatIndex = EditorGUI.Popup(position, label.text, floatIndex, valuesAsString); // Draw popup and get new index selected
                ...
                case SerializedPropertyType.Integer:
                    ...
                    int newIntIndex = EditorGUI.Popup(position, label.text, intIndex, valuesAsString); // Draw popup and get new index selected
                ...
                case SerializedPropertyType.String:
                    ...
                    int newStringIndex = EditorGUI.Popup(position, label.text, stringIndex, valuesAsString); // Draw popup and get new index selected
